...
import { RedisPubSubService } from 'src/redis/redis.pubsub.service';
  ...
  return this.prisma.message.findMany({
    ...
    async createMessage(userId: string, carpoolId: string, dto: CreateMessageDto) {
      ...
      if (hasExpired) throw new ForbiddenException('Carpool conversation expired');
    ...
    async getConversationTray(userId: string) {
      ...
      const senderIds = [...new Set([...messagesByCarpool.values()].map(m => m.senderId))];
    ...
    async getTotalUnreadCount(userId: string) {
        const keys = await this.redisService.client.keys(`message:unread:${userId}:*`);
      ...
      async markMessagesAsRead(userId: string, carpoolId: string): Promise<void> {
        ...
        await this.redisService.client.set(`message:unread:${userId}:${carpoolId}`,  0);
      }
