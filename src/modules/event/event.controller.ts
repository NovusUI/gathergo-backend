...
export class EventController {
  ...
  private parseFlatTicketFields(body: any): CreateEventTicketDto[] {
    console.log(body)
    const ticketFields = Object.keys(body).filter(key => key.startsWith('tickets.'));
  ...
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard)
  @ApiQuery({ name: 'userId', required: false, description: 'Target user ID (defaults to current user)' })
